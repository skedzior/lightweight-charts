...
import { ExpiringPriceAlertsPaneRenderer, type RendererDataItem } from './renderer';
				...
				if (alert.parameters.crossingDirection === 'up') {
					color = alert.crossed
						...
						: alert.expired
							? '#30472C'
							: '#64C750';
					icon = alert.crossed ? tickIcon : alert.expired ? cancelIcon : upArrowIcon;
					...
					color = alert.crossed
						...
						: alert.expired
							? '#4A2D37'
							: '#C83264';
					icon = alert.crossed ? tickIcon : alert.expired ? cancelIcon : downArrowIcon;
				}
				data.push({
					priceY,
					startX,
