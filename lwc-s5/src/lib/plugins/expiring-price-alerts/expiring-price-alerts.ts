...
import { LineSeries, MismatchDirection } from 'lightweight-charts';
import type {
	ExpiringPriceAlert,
...
/**
 * This Plugin works best with a chart which has a linear time scale.
 */
...
export class ExpiringPriceAlerts implements IExpiringPriceAlerts {
	...
	_update() {
		...
		if (start) {
			const lastPlotDate =
				(this._series.dataByIndex(1000000, MismatchDirection.NearestLeft)?.time as
					| number
					| undefined) ?? start;
			if (lastPlotDate < start) start = lastPlotDate;
		}
		if (this._whitespaceSeriesStart !== start || this._whitespaceSeriesEnd !== end) {
			...
			if (!this._whitespaceSeriesStart || !this._whitespaceSeriesEnd) {
				...
				this._whitespaceSeries.setData(
					this._buildWhitespace(this._whitespaceSeriesStart, this._whitespaceSeriesEnd)
				);
			}
		}
