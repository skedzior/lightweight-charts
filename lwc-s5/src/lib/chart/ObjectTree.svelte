<script lang="ts">
	...
	let { chart, objects = $bindable([]), onToggleVisibility, onRemove }: Props = $props();
		...
		{:else}
			{#each [...objectsByPane.entries()] as [paneIndex, paneObjects]}
				<div class="pane-group">
					<button class="pane-header" onclick={() => togglePane(paneIndex)}>
					...
					{#if expandedPanes.has(paneIndex)}
						<div class="pane-objects">
							{#each paneObjects as obj}
								<div class="object-item" class:hidden={!obj.visible}>
